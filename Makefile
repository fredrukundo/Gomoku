...
CXX = c++
CXXFLAGS = -Wall -Wextra -Werror -std=c++17 -Iinclude -Iinclude/thirdparty
CXXFLAGS += $(shell pkg-config --cflags sdl2)
LDFLAGS = $(shell pkg-config --libs sdl2) -l:libSDL2_ttf-2.0.so.0
...
fclean: clean
	rm -f $(NAME) run_tests run_ai_test
